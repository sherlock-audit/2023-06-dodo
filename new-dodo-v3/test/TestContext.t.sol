...
contract TestContext is Test {
    ...
    function userDeposit(address user, address token, uint256 amount) public {
        vm.prank(user);
        d3Proxy.userDeposit(user, token, amount);
    }
    ...
    function contructToken1Dec8MMInfo() public pure returns(MakerTypes.TokenMMInfoWithoutCum memory tokenInfo) {
        tokenInfo.priceInfo = stickPrice(1300, 18, 6, 12, 10);
        tokenInfo.amountInfo = stickAmount(30, 8, 300, 18);
        tokenInfo.kAsk = tokenInfo.kBid = 1000;
        tokenInfo.decimal = 18;
    }
    ...
    function contructBTCMMInfo() public pure returns(MakerTypes.TokenMMInfoWithoutCum memory tokenInfo) {
        ...
        tokenInfo.amountInfo = stickAmount(30, 8, 3000, 18);
        tokenInfo.kAsk = tokenInfo.kBid = 1000;
        tokenInfo.decimal = 8;
    }
    ...
    function poolMakerSetAllTokenInfo() public {
        ...
        vm.startPrank(maker);
        d3MakerWithPool.setNewToken(address(token1), true, token1Info.priceInfo, token1Info.amountInfo, token1Info.kAsk, token1Info.kBid, 8);
        d3MakerWithPool.setNewToken(address(token2), true, token2Info.priceInfo, token2Info.amountInfo, token2Info.kAsk, token2Info.kBid, 18);
        d3MakerWithPool.setNewToken(address(token3), true, token3Info.priceInfo, token3Info.amountInfo, token3Info.kAsk, token3Info.kBid, 18);
        d3MakerWithPool.setNewToken(address(weth), true, token2Info.priceInfo, token2Info.amountInfo, token2Info.kAsk, token2Info.kBid, 18);
        vm.stopPrank();
    }
