...
contract MakerTest is TestContext {
    ...
    function setAllTokenInfo() public {
        ...
        d3Maker.setNewToken(address(token2), true, token1Info.priceInfo, token1Info.amountInfo, token1Info.kAsk, token1Info.kBid, 8);
        d3Maker.setNewToken(address(token3), false, token1Info.priceInfo, token1Info.amountInfo, token1Info.kAsk, token1Info.kBid, 18);
        d3Maker.setNewToken(address(token4), false, token1Info.priceInfo, token1Info.amountInfo, token1Info.kAsk, token1Info.kBid, 8);
        vm.stopPrank();
    ...
    function testSetNewToken() public {
        ...
        assertEq(tokenMMInfo.askUpPrice, 130156 * (10 ** 26));
        assertEq(tokenMMInfo.askDownPrice, 130104 * (10 ** 26));
        assertEq(tokenMMInfo.bidUpPrice, 77000); //1298.7
        assertEq(tokenMMInfo.bidDownPrice, 76984); // 1298.96
        assertEq(tokenMMInfo.askAmount, 30 * (10 ** 18));
        assertEq(tokenMMInfo.bidAmount, 30 * (10 ** 18));
        ...
        vm.prank(owner);
        d3Maker.setNewToken(
            address(token1), true, token1Info.priceInfo, token1Info.amountInfo, token1Info.kAsk, token1Info.kBid, 18
        );
    }
    ...
    function testSetTokenPrice() public {
        ...
        // set 2 token
        {
            ...
            assertEq(tokenMMInfo.askUpPrice, 27913456 * (10 ** 25)); // 27913.456
            assertEq(tokenMMInfo.askDownPrice, 27902304 * (10 ** 25)); // 27902.304
            assertEq(tokenMMInfo.bidUpPrice, 3590); // 1/27852.12, 27880 - 27880 * 0.1% =27852.12
            assertEq(tokenMMInfo.bidDownPrice, 3589); // 1/27857.696
