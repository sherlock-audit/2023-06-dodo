...
contract PMMRangeOrderTest is TestContext {
    ...
    function setUp() public {
        ...
        vm.startPrank(owner);
        d3Maker.setNewToken(address(token1), true, token1Info.priceInfo, token1Info.amountInfo, token1Info.kAsk, token1Info.kBid, 8);
        d3Maker.setNewToken(address(token2), true, token2Info.priceInfo, token2Info.amountInfo, token2Info.kAsk, token2Info.kBid, 18);
        d3Maker.setNewToken(address(token3), true, token3Info.priceInfo, token3Info.amountInfo, token3Info.kAsk, token3Info.kBid, 18);
        vm.stopPrank();
    }
    ...
    function testQuerySellTokens() public {
        ...
        (uint256 fromAmount, uint256 receiveToToken, uint256 vusdAmount) = pmmRangeOrderHelper.querySellTokens(roState, address(token1), address(token2), 10 ** 8);
        assertEq(fromAmount, 10**8);
        assertEq(receiveToToken, 24964525068078312916); // 24.96, because max vusdAmount = 300, suppose 25
        assertEq(vusdAmount, 300 * (10** 18));

        (fromAmount, receiveToToken, vusdAmount) = pmmRangeOrderHelper.querySellTokens(roState, address(token1), address(token2), 10 ** 7);
        assertEq(fromAmount, 10**7);
        //console.log(receiveToToken);
        //console.log(vusdAmount);
        assertEq(receiveToToken, 10812725271096067414); // 10.81, suppose near 10.83
        assertEq(vusdAmount, 129891283405183499858); // 129.8, suppose near 130

        // with cumulative
        roState.fromTokenMMInfo.cumulativeAsk = 4 * (10 ** 8);
        ...
        (fromAmount, receiveToToken, vusdAmount) = pmmRangeOrderHelper.querySellTokens(roState, address(token1), address(token2), 10 ** 8);
        assertEq(fromAmount, 10**8);
        ...
        assertEq(vusdAmount, 129887686605257245795); // 129.8, suppose near 130, < 12989
        ...
        assertEq(receiveToToken, 1998281); // 0.019, suppose near 0.02
        ...
        assertEq(receiveToToken, 921331); // 0.0092, suppose near 0.00923
    ...
    function testQueryBuyTokens() public {
        ...
        assertEq(fromAmount, 23097598); // 0.2309, suppose near(>) 0.2304
        ...
        assertEq(fromAmount, 9999999); // suppose nead 10**7
        ...
        assertEq(fromAmount, 20018196); // suppose near(>) 0.02
        assertEq(receiveToToken, 21633407960311327528); // 21.63, because max vusdAmount = 260, suppose near 21.67
        ...
        assertEq(fromAmount, 9999999); // suppose 10 ** 7
        assertEq(receiveToToken, 10810278971047276557); // 10.81, suppose near 10.83 < 10812
        ...
        (fromAmount, receiveToToken, vusdAmount) = pmmRangeOrderHelper.queryBuyTokens(roState, address(token2), address(token1), 1998281);
        assertEq(fromAmount, 2169246371029330793); //21.6, suppose near(>) 21.6
        assertEq(receiveToToken, 1998281); // 0.019, suppose near 0.02
        assertEq(vusdAmount, 25999820461673340372); //suppose near 26 * (10** 18)), max vusdAmount = 26

        (fromAmount, receiveToToken, vusdAmount) = pmmRangeOrderHelper.queryBuyTokens(roState, address(token2), address(token1), 921331);
        assertEq(fromAmount, 999992749573522228); // suppose 1e18
        assertEq(receiveToToken, 921331); // 0.0092, suppose near 0.00923
        assertEq(vusdAmount, 11987522711348296876); // suppose 11987609614474078065,  11.9
    }
